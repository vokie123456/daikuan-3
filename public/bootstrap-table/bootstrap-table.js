...
(function ($) {
        ...
        if (this.options.search) {
            ...
            html.push(
                ...
                sprintf('<input class="form-control' +
                    ...
                    this.options.formatSearch(this.options.searchPlaceholder)),
                '</div>');
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if (!this.options.onlyInfoPagination) {
            ...
            if(this.options.showInputPagination) {
                inputPagination = 
                '<div class="pull-right input-pagination">' +
                    '<span class="inputPaginationTitle">去</span>' +
                    '<input class="form-control marginLR4 inputPagination" type="text">' +
                    '<span>页</span>' +
                    '<button class="btn btn-default btn-sm marginLR4 btnInputPagination">GO</button>'
                '</div>';
            }
            // 自定义 end
    ...
    BootstrapTable.prototype.initRow = function(item, i, data, parentDom, isRowTitle = false) {
        var that=this,
            key,
            html = [],
            style = {},
        ...
        //自定义
        if(this.options.showRowTitle && !!this.options.rowTitleFormatter && isRowTitle) {
            var row_str = this.options.rowTitleFormatter(null, item, i);
            html.push(sprintf('<td class="rowTitleStyle" colspan="%s">', this.header.fields.length) + row_str + '</td>');
            return html.join(' ');
        }
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            var item = data[i];
            var tr = this.initRow(item, i, data, trFragments, false);
            //自定义 start
            if(that.options.showRowTitle) {
                tr = this.initRow(item, i, data, trFragments, true) + tr;
            }
            //自定义 end
            hasTr = hasTr || !!tr;
            if (tr&&tr!==true) {
